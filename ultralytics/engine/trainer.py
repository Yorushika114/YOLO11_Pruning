...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        if RANK in {-1, 0}:
            ...
            YAML.save(self.save_dir / "args.yaml",
    ...
    def _setup_scheduler(self):
        ...
        if self.args.cos_lr:
            ...
            self.lf = lambda x: max(
                # linear
                1 - x / self.epochs, 0) * (1.0 - self.args.lrf) + self.args.lrf
    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            criterion = getattr(model, "None", None)
            if criterion is not None and hasattr(criterion, "set_epoch_info"):
                criterion.set_epoch_info(epoch, self.epochs)

            self.run_callbacks("on_train_epoch_start")
           # model = self.model.module if hasattr(
           #     self.model, "module") else self.model
           # if hasattr(model, "loss"):
           #     model.loss.epoch = epoch
           #     model.loss.total_epochs = self.epochs
