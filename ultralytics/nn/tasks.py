...
class BaseModel(torch.nn.Module):
    ...
    def is_fused(self, thresh=10):
        ...
        """
        bn = tuple(v for k, v in torch.nn.__dict__.items()
...
class ClassificationModel(BaseModel):
    ...
    def _from_yaml(self, cfg, ch, nc, verbose):
        ...
        # Define model
        ch = self.yaml["channels"] = self.yaml.get(
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        model = self.clip_model if cache_clip_model else build_text_model(
            "clip:ViT-B/32", device=device)
...
class YOLOEModel(DetectionModel):
    ...
    def set_vocab(self, vocab, names):
        ...
        self(torch.empty(1, 3, self.args["imgsz"],
             self.args["imgsz"]).to(device))  # warmup
    ...
    def get_vocab(self, names):
        ...
        device = next(self.model.parameters()).device
        # fuse prompt embeddings to classify head
    ...
    def loss(self, batch, preds=None):
        ...
        if not hasattr(self, "criterion"):
            ...
            self.criterion = TVPDetectLoss(
                self) if visual_prompt else self.init_criterion()

        if preds is None:
            preds = self.forward(batch["img"], tpe=batch.get(
                "txt_feats", None), vpe=batch.get("visuals", None))
        return self.criterion(preds, batch)
...
def temporary_modules(modules=None, attributes=None):
    ...
    try:
        ...
        for old, new in attributes.items():
            ...
            setattr(import_module(old_module), old_attr,
    ...
    try:
        with temporary_modules(
            ...
            if safe_only:
                ...
                safe_pickle.load = lambda file_obj: SafeUnpickler(
                    file_obj).load()
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in base_modules:
            ...
            if m is C2fAttn:  # set 1) embed channels and 2) num heads
                ...
                args[2] = int(max(round(
                    min(args[2], max_channels // 2 // 32)) * width, 1) if args[2] > 1 else args[2])
            ...
            c2 = sum(ch[x] for x in f)
                ...
                {Detect, WorldDetect, YOLOEDetect, Segment,
                    YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
